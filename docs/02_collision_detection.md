...
It is responsible for determining whether two shapes are colliding and if so, generating the necessary contact data for collision resolution.
...
This demo allows you to visualize the different collision information that is generated by the engine.
...
After the broad phase has been executed, the narrow phase is responsible for performing detailed collision detection on the remaining candidate pairs identified by the broad phase.
We will first discuss the broad phase implementations available in the engine, followed by the narrow phase collision detection for different types of shapes.
...
The formal numerical complexity of this method is still $O(n^2)$ since it checks all pairs of objects, however, the amount of compute per checked pair is much lower than a full narrow phase check on all of the body pairs.
...
We store this information in the `Contact` dataclass implemented in [`ppe/engine/common.py`](../ppe/engine/common.py).

### Handling Different Types of Shapes with a Dispatch System
...
Implementation detail: To make sure that a circle-polygon pair is handled the same way as a polygon-circle pair, the dispatch system always orders the shapes by an numeric type ID before looking up the handler.
...
- **Collision Normal**: A unit vector that points outwards from the reference shape. It indicates the direction in which the shapes should be separated.
...
The whole process is probably best understood by looking at this image:

<!-- ![Circle vs Circle Collision](assets/circle_circle_collision.svg) -->
<img src="assets/circle_circle_collision.svg" alt="Circle vs Circle Collision" style="width: 70%; display: block; margin: auto">
...
### Circle vs Polygon
The main idea of the circle-vs-polygon collision detection algorithm we use in this library is to find the closest point on the polygon to the center of the circle.
...
Again, its probably best understood by visualizing the geometry.
...
<img src="assets/circle_polygon_collision_2.svg" alt="Circle vs Polygon Collision" style="width: 70%; display: block; margin: auto">
...
The concrete implementation in this engine (`SatPolygonHandler`) can be found in [`ppe/engine/collision_handlers.py`](../ppe/engine/collision_handlers.py).
...
Instead of checking all possible axes (which would be computationally impossible ;)), it's sufficient to check all the axes that are perpendicular to the polygon's edges.
...
However, we can not simply pick the first axis on which an overlap is found. 
...
Instead, we need to "clip" the incident edge against the extended side planes of the reference edge.
...
<img src="assets/clipping_normal.svg" alt="Clipping Visualization" style="width: 60%; display: block; margin: auto">
...
<img src="assets/clipping_calculation.svg" alt="Narrow image example" style="width: 60%; display: block; margin: auto">
...
The algorithm remains the same but it shows why its important to always clip against both side planes.
![SAT Contact Points Edge-on-Edge](assets/clipping_edge_on_edge.svg)

The clipping process always gives us two potential contact points.
However, in most cases, only one of them is actually penetrating the reference edge.
Therefore, we need to filter the resulting clipped points to only keep those that are actually penetrating the reference edge.
...
Finally, the following pseudocode combines all these steps:
```python
